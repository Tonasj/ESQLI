...
def find_project_root():
    """
    Find the project root during development.
    For PyInstaller builds, this will just return the current directory.
    """
    try:
        return sys._MEIPASS
...
def setup_app_settings():
    ...
    - Uses `ESQLI/config/` when developing.
    - Uses a writable `~/.esqli/config/` folder in packaged builds.
    """
    # Determine where to store config safely
    if getattr(sys, 'frozen', False):
        # PyInstaller executable: use user home directory
        base_folder = os.path.join(os.path.expanduser("~"), ".esqli")
    else:
...
# --- Window Persistence (Geometry + Splitters) ---
...
# --- Optional decorator for automatic persistence ---
